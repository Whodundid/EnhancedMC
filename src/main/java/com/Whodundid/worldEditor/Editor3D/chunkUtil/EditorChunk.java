...
public class EditorChunk {
	...
	public EditorChunk(EditorChunkCache cache) { this(cache, new Vector3DInt()); }
	public EditorChunk(EditorChunkCache cache, Vector3DInt startPosIn) {
		chunks = cache;
		startPos = startPosIn;
		startX = startPos.x;
		startY = startPos.y;
		...
		for (int x = startPos.x; x < endX; x++) {
			for (int z = startPos.z; z < endZ; z++) {
				for (int y = startPos.y; y < endY; y++) {
					blockList.add(new EditorBlock(x, y, z, EditorMaterial.AIR, this));
				}
			}
		}
	}
		...
		for (int x = startPos.x; x < endX; x++) {
			for (int z = startPos.z; z < endZ; z++) {
				for (int y = startPos.y; y < endY; y++) {
					...
					blockList.add(new EditorBlock(x, y, z, m, this));
				}
			}
		}
	}
