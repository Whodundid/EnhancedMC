...
public class EditorBlock {
	...
	public EditorBlock(int x, int y, int z, EditorMaterial materialIn, EditorChunk chunkIn) { this (new Vector3DInt(x, y, z), new Vector3D(1, 1, 1), materialIn, chunkIn); }
	public EditorBlock(Vector3DInt positionIn, Vector3D dimensions, EditorMaterial materialIn, EditorChunk chunkIn) {
		chunk = chunkIn;
		position = positionIn;
	...
	public void createVertexData() {
		if (!material.equals(EditorMaterial.AIR)) {
			...
			for (EditorDirection d : renderedFaces) {
				switch (d) {
				case DOWN: 
					vertexData.add(new DPolygon(new double[] {x, x + width, x + width, x},
												new double[] {y, y, y + length, y + length},
												new double[] {z, z, z, z},
												c, this, EditorDirection.DOWN));
					break;
				case EAST: 
					vertexData.add(new DPolygon(new double[] {x + width, x + width, x + width, x + width},
												new double[] {y, y, y + length, y + length},
												new double[] {z, z + height, z + height, z},
												c, this, EditorDirection.EAST));
					break;
				case NORTH: 
					vertexData.add(new DPolygon(new double[] {x, x, x + width, x + width}, 
				...
				case UP: 
					...
					break;
				case WEST:
					vertexData.add(new DPolygon(new double[] {x, x, x, x},
												new double[] {y, y, y + length, y + length},
												new double[] {z, z + height, z + height, z},
	...
	private void determineRenderedFaces() {
		EditorBlock up = chunk.getBlockAtPos(position.x, position.y + 1, position.z);
		EditorBlock down = chunk.getBlockAtPos(position.x, position.y - 1, position.z);
		EditorBlock north = chunk.getBlockAtPos(position.x, position.y, position.z - 1);
		EditorBlock east = chunk.getBlockAtPos(position.x - 1, position.y, position.z);
		EditorBlock south = chunk.getBlockAtPos(position.x, position.y, position.z + 1);
		EditorBlock west = chunk.getBlockAtPos(position.x + 1, position.y, position.z);
