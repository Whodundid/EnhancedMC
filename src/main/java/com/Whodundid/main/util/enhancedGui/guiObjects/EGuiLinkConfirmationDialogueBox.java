...
public class EGuiLinkConfirmationDialogueBox extends EGuiDialogueBox {
	...
	public EGuiLinkConfirmationDialogueBox(IEnhancedGuiObject parentIn, String linkIn) {
		...
		int longestString = 0;
		longestString = MainMod.getFontRenderer().getStringWidth(prompt);
		int linkLength = MainMod.getFontRenderer().getStringWidth(linkIn);
		if (linkLength > longestString) { longestString = linkLength; }
		int copyLength = MainMod.getFontRenderer().getStringWidth(I18n.format("chat.copy", new Object[0])) + 8;
		...
		EGuiLabel promptLabel = new EGuiLabel(this, res.getScaledWidth() / 2, y + 15, prompt).setDrawCentered(true).setDisplayStringColor(0xffbb00);
		EGuiLabel linkLabel = new EGuiLabel(this, res.getScaledWidth() / 2, y + 30, linkIn).setDrawCentered(true).enableWordWrap(true, longestString);
		EGuiLabel warningLabel = new EGuiLabel(this, res.getScaledWidth() / 2, y + 35 + linkLabel.getTextHeight(), warning).setDrawCentered(true).setDisplayStringColor(0xff5555);
