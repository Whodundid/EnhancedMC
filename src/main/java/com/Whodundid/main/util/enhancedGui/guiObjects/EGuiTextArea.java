...
public class EGuiTextArea<obj> extends EnhancedGuiObject {
	...
	ArrayList<TextAreaLine> textDocument, drawnLines;
	...
	public EGuiTextArea(IEnhancedGuiObject parentIn, int x, int y, int widthIn, int heightIn, boolean editableIn) {
		...
		horizontalScroll = new EGuiScrollBar(this, 0, 0, false, Direction.S) {
			{ setVisible(false); }
			@Override
			public void mouseReleased(int mX, int mY, int button) {
				getParent().requestFocus();
		...
		//System.out.println("verticalScroll: " + verticalScroll.getScrollPos());
			...
			synchronized (guiObjects) {
				for (int i = 0; i <= getTopParent().getHighestZLevel(); i++) {
					...
					while (it.hasNext()) {
						...
						if (o.checkDraw() && o.getZLevel() == i && !textDocument.contains(o)) {
		    				GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
	...
	private TextAreaLine insertTextLine(int pos) {
		ArrayList<TextAreaLine> linesAfterPos = new ArrayList();
		...
		try {
			...
			while (pos < textDocument.size() && pos < (currentPosY + getLineCount())) {
				...
				if (l != null) {
					...
					l.setDimensions(xPos, (i * lineHeight) + (startY + 2), widthVal, lineHeight);
	...
	public ArrayList<TextAreaLine> getTextDocument() { return textDocument; }
}
