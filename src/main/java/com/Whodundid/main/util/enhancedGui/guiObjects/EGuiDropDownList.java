...
import com.Whodundid.main.util.enhancedGui.guiObjectUtil.DropDownListEntry;
...
public class EGuiDropDownList extends EnhancedGuiObject {
	...
	public void onFocusGained(FocusEvent eventIn) {
		if (eventIn.type.equals(FocusType.mousePress)) {
			if (listOpen) {	selectListOption(eventIn.actionCode); }
			else { openList(eventIn.actionCode); }
		}
		getTopParent().setFocusLockObject(this);
	}
	
	@Override public void onFocusLost(FocusEvent eventin) { closeList(); }
